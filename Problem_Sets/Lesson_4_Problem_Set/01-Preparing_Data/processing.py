...
FIELDS ={'rdf-schema#label': 'label',
         ...
         'genus_label': 'genus'}
CLASSIFY = ['family','class','phylum','order','kingdom','genus']
...
def process_file(filename, fields):
    ...
    with open(filename, "r") as f:
        ...
        for line in reader:
            ...
            # YOUR CODE HERE
            # keys of the dictionary changed according to the mapping in FIELDS dictionary
            for item in process_fields:
                if item in line:
                    oldkey = item
                    newkey = fields[item]
                    if newkey in CLASSIFY:
                        temp['classification'][newkey] = line[item]
                    else:
                        temp[newkey] = line[item]

                # trim out redundant description in parenthesis from the
                # 'rdf-schema#label' field, like "(spider)"
                if newkey == 'label':
                    temp[newkey] = re.search('\S+',line[item]).group()

                if newkey == 'name':
                    if (temp[newkey] == ('NULL' or re.search('\W',temp[newkey] ).group())):
                        temp[newkey] = temp['label']

                if ((newkey not in CLASSIFY) and (temp[newkey] == 'NULL') and (temp[newkey] != None)):
                    temp[newkey] = None

                if ((newkey in CLASSIFY) and (temp['classification'][newkey] == 'NULL') ):
                    temp['classification'][newkey] = None

                if ((newkey == 'synonym') and (temp[newkey] != None) and (temp[newkey].startswith('{')) ):
                    temp[newkey] = temp[newkey].strip('{}')
                    temp[newkey] = re.split('\|',re.sub('\* ','',temp[newkey]))
                elif((newkey == 'synonym') and (temp[newkey] != None) ):
                    temp[newkey] = re.split('',temp[newkey])
                else:
                    pass
